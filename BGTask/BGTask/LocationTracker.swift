...
//        for (var i : Int = 0; i < locations.count; i += 1) {
...
//        for (var i : Int = 0 ; i < self.shareModel!.myLocationArray!.count ; i+=1) {
        if let count = self.shareModel?.myLocationArray?.count {
            for i in 0..<count {
        ...
        print("Send to server: latitude \(String(describing: self.myLocation?.latitude)) longitude \(String(describing: self.myLocation?.longitude)) accuracy \(String(describing: self.myLocationAcuracy))\n")
...
//        for (var i : Int = 0 ; i < self.shareModel!.myLocationArray!.count ; i += 1) {
        if let count = self.shareModel?.myLocationArray?.count {
            for i in 0..<count {
                var currentLocation : NSMutableDictionary = self.shareModel!.myLocationArray!.object(at: i) as! NSMutableDictionary
                if i == 0 {
                    myBestLocation = currentLocation
                } else {
                    if (currentLocation.object(forKey: ACCURACY) as! Float) <= (myBestLocation.object(forKey: ACCURACY) as! Float) {
        ...
        if self.shareModel!.myLocationArray!.count == 0 {
            ...
            let lat : CLLocationDegrees = myBestLocation.object(forKey: LATITUDE) as! CLLocationDegrees
            var lon : CLLocationDegrees = myBestLocation.object(forKey: LONGITUDE) as! CLLocationDegrees
